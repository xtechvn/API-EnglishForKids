...
{
    public class CourseService : ESRepository<CourseModel>
    {
       
        private readonly IConfiguration configuration;
        private static string _ElasticHost;
        private static ElasticClient elasticClient;
        private ISearchResponse<CourseModel> search_response;
        public CourseService (string Host, IConfiguration _configuration) : base(Host, _configuration)
        {
            _ElasticHost = Host;
        ...
        public ArticleModel GetArticleDetailById(long id)
        {
            try
            {
                ...
                var query = elasticClient.Search<ArticleModel>(sd => sd
                 .Index(configuration["DataBaseConfig:Elastic:Index:SP_GetAllSource"])  // Chỉ mục bạn muốn tìm kiếm
               .Query(q => q
        ...
        /// <returns></returns>
        public List<CourseModel> getListCourse(int category_id, int top)
        {
            ...
            try
            {
                ...
                if (category_id <= 0)
                {
                    ...
                    search_response = elasticClient.Search<CourseModel>(s => s
                   .Size(top) // Lấy ra số lượng bản ghi (ví dụ 100)
                   .Index(configuration["DataBaseConfig:Elastic:Index:SP_GetAllSource"])  // Chỉ mục bạn muốn tìm kiếm
                       .Sort(sort => sort
                           .Descending(f => f.created_date) // Sắp xếp giảm dần theo publishdate
                       )
                   );
                }
                else
                { 
                    //search_response = elasticClient.Search<CourseModel>(s => s
                    //    .Size(top)
                    //    .Index(configuration["DataBaseConfig:Elastic:Index:SP_GetListLessionBySourceId"])  // Chỉ mục muốn tìm kiếm
                    //    .Sort(sort => sort
                    //        .Descending(f => f.createdate)
                    //    )
                    //   .Query(q => q
                    //        .Bool(b => b
                    //            .Must(m => m
        ...
        public int getTotalItemCoursesByCategoryId(int category_id)
        {
            try
            {
                int totalCount = 0;
                ...
                if (category_id > 0)
                {
                    var countResponse = elasticClient.Count<CategoryCourseModel>(c => c
                    .Index(configuration["DataBaseConfig:Elastic:Index:SP_GetAllSource"])  // Chỉ mục bạn muốn tìm kiếm
                    .Query(q => q
                ...
                {
                    var countResponse = elasticClient.Count<CategoryCourseModel>(c => c.Index(configuration["DataBaseConfig:Elastic:Index:SP_GetAllSource"]));
                    totalCount = Convert.ToInt32(countResponse.Count);
                }
